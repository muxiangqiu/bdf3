...
public class MessageServiceImpl implements MessageService {
	...
	public List<Notify> loadUnread(String user) {
		...
		List<Notify> notifies = JpaUtil.linq(Notify.class)
				...
				.findAll();
		if (!notifies.isEmpty()) {
			Set<String> senders = JpaUtil.collect(notifies, "sender");
			List<User> users = JpaUtil.linq(User.class).in("username", senders).findAll();
			Map<String, User> userMap = JpaUtil.index(users);
			for (Notify notify : notifies) {
				notify.setAdditional(userMap.get(notify.getSender()));
			}
		}
		return notifies;
	}
